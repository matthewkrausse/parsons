"""Tests for the APIConnector features added on top of the request helpers.
...
OAuth2 token-expiry retry. The `*_request` helpers are covered in
...
def test_get_returns_response(requests_mock, connector):
...
def test_post_sends_json_and_returns_response(requests_mock, connector):
...
def test_put_patch_delete(requests_mock, connector):
...
def test_delete_accepts_body(requests_mock, connector):
    # delete_request takes no body; the verb method does.
...
def test_verbs_raise_typed_errors(requests_mock, connector):
...
def test_per_request_headers_merge_over_connector_headers(requests_mock):
...
def test_kwargs_passed_through(requests_mock, connector):
    requests_mock.get(BASE_URI + "things", json={})
...
def test_default_is_no_timeout(requests_mock, connector):
    requests_mock.get(BASE_URI + "things", json={})
    connector.get("things")
    assert requests_mock.last_request.timeout is None


def test_connector_timeout_used(requests_mock):
...
def test_per_request_override(requests_mock):
    conn = APIConnector(BASE_URI, timeout=(10, 120))
    requests_mock.get(BASE_URI + "things", json={})
...
def test_per_request_none_disables(requests_mock):
    conn = APIConnector(BASE_URI, timeout=(10, 120))
    requests_mock.get(BASE_URI + "things", json={})
...
def test_429_raises_rate_limit_error_with_retry_after(requests_mock, connector):
    requests_mock.get(
        BASE_URI + "things",
        status_code=429,
...
def test_retry_after_http_date_form(requests_mock, connector):
    requests_mock.get(
        BASE_URI + "things",
        status_code=429,
        reason="Too Many Requests",
...
def test_retry_after_zoneless_http_date(requests_mock, connector):
...
def test_retry_after_absent(requests_mock, connector):
...
def test_401_raises_authentication_error(requests_mock, connector):
    requests_mock.get(BASE_URI + "things", status_code=401, reason="Unauthorized")
    with pytest.raises(AuthenticationError):
        connector.get("things")


def test_typed_errors_are_http_errors(requests_mock, connector):
...
def test_legacy_methods_raise_typed_errors_too(requests_mock, connector):
    requests_mock.get(BASE_URI + "things", status_code=429, reason="Too Many Requests")
...
def test_int_builds_default_policy():
...
def test_custom_retry_object_honored():
...
def test_default_retry_never_retries_post():
...
def test_sleeps_between_requests(requests_mock, monkeypatch):
    sleeps = []
...
def test_no_throttle_by_default(requests_mock, monkeypatch):
    sleeps = []
    monkeypatch.setattr(api_connector_module, "_sleep", sleeps.append)
    conn = APIConnector(BASE_URI)
    requests_mock.get(BASE_URI + "things", json={})
...
def test_link_header_pagination(requests_mock, connector):
...
def test_next_url_pagination_with_dotted_key(requests_mock, connector):
...
def test_link_header_relative_url_resolves_against_page(requests_mock, connector):
...
def test_next_url_relative_resolves_against_page(requests_mock, connector):
    requests_mock.get(
        BASE_URI + "things",
        [
...
def test_cursor_pagination_preserves_params(requests_mock, connector):
    requests_mock.get(
        BASE_URI + "things",
        [
...
def test_cursor_pagination_stops_on_string_more_flag(requests_mock, connector):
    ...
    # max_pages bounds the loop so a regression in the stop condition fails
    # the assertion below instead of paginating forever (requests_mock
    # replays its last response indefinitely).
    pages = list(
        connector.paginate(
            "things",
...
def test_page_number_pagination_stops_on_empty_page(requests_mock, connector):
    requests_mock.get(
        BASE_URI + "things",
        [
            {"json": {"results": [1, 2]}},
...
def test_page_number_stops_on_more_flag(requests_mock, connector):
    requests_mock.get(
        BASE_URI + "things",
        [
...
def test_page_number_short_page_saves_a_request(requests_mock, connector):
    requests_mock.get(
        BASE_URI + "things",
        [
            {"json": [1, 2, 3]},
...
def test_max_pages_safety_valve(requests_mock, connector):
    requests_mock.get(
        BASE_URI + "things",
...
def test_pagination_errors_propagate(requests_mock, connector):
    requests_mock.get(BASE_URI + "things", status_code=500, reason="Server Error")
    with pytest.raises(ParsonsHTTPError):
        list(connector.paginate("things", LinkHeaderPaginator()))


# O Auth2 Token Expiry Retry

TOKEN_URL = "https://auth.example.com/oauth/token"


def test_expired_token_is_refetched_and_request_retried(requests_mock):
    requests_mock.post(
        TOKEN_URL,
        [
    ...
    token_fetches = [r for r in requests_mock.request_history if r.url == TOKEN_URL]
    assert len(token_fetches) == 2


def test_post_is_not_duplicated_on_token_refresh(requests_mock):
