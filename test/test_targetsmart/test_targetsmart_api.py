...
def test_api_key_sent_as_custom_header(ts_api, output_list, requests_mock):
    # The migration routes requests through APIConnector + HeaderTokenAuth;
    # the API key must still be sent as the bare x-api-key header (no scheme).
    requests_mock.get(ts_api.connection.uri + "person/data-enhance", json={"output": output_list})
