...
- Migrating an existing connector? See the [migration guide](../../docs/api_connector_migration.md).
...
| An API key in the query string | `params=` / base uri | key stays a query param, no auth object |
...
expiry. Otherwise it has the exact same surface as `APIConnector`.
...
### Legacy `*_request` methods

`get_request`, `post_request`, `put_request`, `patch_request`, and
`delete_request` predate the verb methods and are **kept unchanged** for the
many existing connectors that use them. They return *parsed* results, not a
`Response`:
...
```python
# stop when the response's "more" boolean is falsy (most precise)
PageNumberPaginator(page_param="page", data_key="results", more_key="more")
...
- **`retries`** — pass an int for the standard policy (`default_retry(total)`)
  ...
  idempotent methods** (`GET`, `HEAD`, `OPTIONS`) — `POST` is never auto-retried,
  because a duplicate `POST` (a donation, a signup) can be worse than a failed
  one. Retries are invisible to callers except that transient failures now
  succeed; the same `HTTPError` is raised if they're exhausted.
- **`rate_limit_interval`** — enforce a minimum gap between requests for APIs
  with strict rate limits (replaces hand-rolled `time.sleep()` calls).
